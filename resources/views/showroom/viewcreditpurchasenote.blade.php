...
<html lang="en">
  <head>
    ...
    <title>Credit Note Purchase</title>
  </head>
  <style type="text/css">
...
$returnNumber = isset($salesReturnData->sales_return_no) ? str_pad($salesReturnData->sales_return_no, 6, '0', STR_PAD_LEFT) : '';
...
<div class="Sales-return-cumcredit width-100 m-lr-auto margin-top-2  border-extra-light bg-extra-light">
  ...
  <header>
    <div class="logo text-center padding-b-one">
      <img src="{{ URL::to('/') }}/img/sales_return_logo.png" class="width-200px"/>
          ...
          <tbody>
            <tr>
              ...
              <td colspan="3">
                ...
                <p>Date: <?= $date?></p>
                <p>Reference  Purchase Invoice No: <?= $invoiceNumber?></p>
              </td>
            </tr>
            <tr>
            ...
            <tr class="text-center">
                <td class="text-center border-top-0">Sr.No</td>
                <td class="text-center border-top-0">Description of good</td>
                <td class="text-center border-top-0">HSN/SAC Code</td>
                <td class="text-center border-top-0">Qty (in pcs)</td>
                <td class="text-center border-top-0">Unit Price</td>
                <td class="text-center border-top-0">Total</td>
            </tr>
            ...
            <tr>
                ...
                <td class="text-center"><?= isset($product->unit_price) ? ShowroomHelper::currencyFormat(round($product->unit_price)) : ''?></td>
                <td class="text-center"><?= isset($product->total) ? ShowroomHelper::currencyFormat(round($product->total)) : ''?></td>
            </tr>
            <?php endforeach;?>
            <tr>
              <td colspan="6" align="right"> Grand Total: <?= isset($salesReturnData->total_invoice_value) ? ShowroomHelper::currencyFormat(round($salesReturnData->total_invoice_value)) : 0?></td>
            </tr>
            <tr>
              <td class="border-top-0" colspan="6">
                <p>Total Taxable Value: <?= isset($salesReturnData->total_taxable_value) ? ShowroomHelper::currencyFormat(round($salesReturnData->total_taxable_value)) : 0?></p>
                <?php if(isset($gstData->igst)):?>
                <p>IGST 3%: <?= ShowroomHelper::currencyFormat(round($gstData->igst))?></p>
                <?php elseif(isset($gstData->cgst)):?>
                <p>CGST 3%: <?= ShowroomHelper::currencyFormat(round($gstData->cgst))?></p>
                <?php elseif(isset($gstData->sgst)):?>
                <p>SGST 3%: <?= ShowroomHelper::currencyFormat(round($gstData->sgst))?></p>
                <?php endif;?>
                <p>Rounding : +0.22/ -0.81</p>
                <p>Invoice Value: <?= isset($salesReturnData->total_invoice_value) ? ShowroomHelper::currencyFormat(round($salesReturnData->total_invoice_value)) : 0?></p>
              </td>
            </tr>
            <tr>
              <td colspan="6" style="border-top:0px;">
                <p class="margin-zero">Total Amount In Words :- <?php echo ucfirst(InventoryHelper::convertNumberToWords(round($salesReturnData->total_invoice_value,0)))?> only</p>
              </td>    
            </tr>
            <tr>
                <td colspan="6">
            ...
            <tr>
              <td colspan="3" class="text-left align-top border-right">Receiver Sign & Seal</td>
              <td colspan="3" class="align-top border-right">

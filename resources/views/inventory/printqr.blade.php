            ...
            <div class="middle" id="anchor-content">
            	<div id="page:main-container">
					...
					body{font-family: Verdana,Geneva, sans-serif!important;}
					  ...
					  .qrcode-print2 .st1 p{text-align:left !important;padding-left: 3px;font-size:5.9pt;}
...
foreach ($qrData as $qrDatakey => $qr) {
	...
	//var_dump($qrData);
	$diamondsData = array();
	$diamondsData[0]['total_diamond_weight'] = (float) 0;
	foreach ($qr['diamonds'] as $diamondKey => $diamond) {

		$diamondsData[$diamondKey]['num'] = $diamondKey + 1;
		$diamondsData[$diamondKey]['shape'] = $sortform[$diamond['shape']];
		$diamondsData[$diamondKey]['totalcts'] = $diamond['totalcts'];
		$diamondsData[$diamondKey]['mm_size'] = $diamond['mm_size'];
		if (!empty($diamond['carat'] && $diamond['carat'] > 0)) {
			$diamondsData[$diamondKey]['total_single_weight'] = number_format((float) round($diamond['carat'] * $diamond['stone_use'], 3), 3, '.', '');
			//var_dump($diamondKey);exit;
			$diamondsData[0]['total_diamond_weight'] += $diamondsData[$diamondKey]['total_single_weight'];
		}
	}
	?>
					<div class="print-qr">
						<div class="qrcode-print1">
							...
							<p class="pricetag">D.WT: <?php if (!empty($qr['diamond_quality'])) {echo $qr['diamond_quality'] . ' | ';}?><?=number_format((float) $diamondsData[0]['total_diamond_weight'], 3, '.', '');?></p>
							<p class="pricetag">M.WT: <?=$Sortmquality?> | <?=round($qr['metal_weight'], 2)?></p>
							<p class="pricetag">Price: <?=money_format('%!.0n', $qr['price'])?></p>
						</div>
						<div class="qrcode-print2">
							<div class="st1">
								<?php
foreach ($diamondsData as $diamondKey => $diamond) {
		?>
								<p><?=$diamondsData[$diamondKey]['num']?>:<?=$diamondsData[$diamondKey]['shape']?><?php if (!empty($diamondsData[$diamondKey]['totalcts'] && $diamondsData[$diamondKey]['totalcts'] > 0)) {echo '|' . $diamondsData[$diamondKey]['total_single_weight'];}if (!empty($diamondsData[$diamondKey]['mm_size'])) {echo '(' . $diamondsData[$diamondKey]['mm_size'] . ')';}?></p>
											<?php
}
	?>
							</div>
							<span>
								<?php
$qrImage = ShowroomHelper::getProductQRImage($qr['certificate_no'], '50');
	?>
								<img src="<?=$qrImage?>">
							</span>
						</div>
